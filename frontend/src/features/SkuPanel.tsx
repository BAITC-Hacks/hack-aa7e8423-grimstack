...
import { formatMoney, formatNumber, formatQty, formatSignedQty } from '../shared/format';
...
export function SkuPanel({ line, runId, onClose }: { line: OrderLine; runId: string; onClose: () => void }) {
  ...
  return <aside className={styles.panel} aria-labelledby="sku-heading">
    ...
    <section aria-labelledby="steps-heading"><h3 id="steps-heading">Почему столько</h3><div className={styles.steps}>{line.components.filter((component) => component.kind === 'qty').map((component) => <div className={styles.step} key={component.key}><span title={component.note ?? undefined}>{component.label}</span><strong>{formatSignedQty(component.value, line.unit)}</strong></div>)}<div className={`${styles.step} ${styles.total}`}><span>Рекомендация</span><strong>{formatQty(line.recommended_qty, line.unit)}</strong></div></div></section>
