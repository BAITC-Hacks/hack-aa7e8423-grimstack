...
Сервер запускается командой `python -m app`. Все маршруты данных имеют префикс `/api`; `GET /health` возвращает `{"status":"ok"}`. Формы ответа заданы в `app/contracts.py`, а TypeScript-зеркало — в `frontend/src/api/types.ts`. Авторизации нет. Прогоны и загруженные наборы хранятся в памяти процесса, утверждения — в `var/approvals.json`.
...
| `POST` | `/api/runs/{run_id}/suppliers/{supplier}/approve` | `supplier`: `IEK` или `SE` | `SupplierSummary` со статусом `approved`; запись утверждения на диск |
| `GET` | `/api/runs/{run_id}/export.xlsx?supplier=SE` | Поставщик | XLSX с положительными `final_qty`; скачивание не утверждает заказ |
| `GET` | `/api/sku/{supplier}/{sku}/history` | Поставщик и код 1С | `SkuHistory` с сырым, очищенным и восстановленным рядом |
...
| 409 | `conflict` | Повторное утверждение или правка утверждённого заказа |
...
Память сервиса не разделяется между несколькими процессами. После перезапуска идентификаторы старых прогонов и наборов недоступны. Маршрут истории SKU не принимает `run_id`: для загруженного набора он использует последний рассчитанный набор процесса, поэтому при одновременной работе нескольких пользователей история может относиться к другому набору. Разворачивать эту версию следует одним процессом.
