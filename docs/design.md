...
requirements.txt, Dockerfile, .env.example                            [Б]
README.md                                                             [Ф]
docs/                                                                 [А]
```
...
2. **Разовые крупные строки** (транзакции 2025+). У SKU должно быть не меньше 5 строк,
   ...
   Строку обрезаем до `max(5·мед, Q3+3·IQR, M_nz)`, а не удаляем. Срезанный излишек
   вычитаем из ячейки помесячного ряда, но не ниже 0. Документ с ≥ 3 отмеченными
...
|---|---|---|
| MH1 в пути | IEK 010500006_ ВА47-29 16А | Крупнейший SKU, в пути 30 000: обнулите «в пути» — заказ вырастет |
| MH1 в пути | SE 030200193_ IMT35150 | В пути 37 800 = 10 × кратность 3 780 |
| MH2 сезон | IEK 130300792_ Труба Ø50 | Октябрь 8 685 → 13 414, январь ~600; группа 1303 с размахом 4.4× |
| MH2 рост | SE 030200201_ IMT35101 | 148 911 (2024) → 231 760 (2025) → +29 % в 2026 |
| MH3 stockout | IEK 010300096_ УЗО АВДТ 32 | Май 2025: строка на 630 шт → июнь начался с 0 → «разовый заказ, затем stockout» |
| MH3 stockout | IEK 130200124_ Шина ШНИ-14 | Остаток 0 в феврале–апреле 2025, продажи 0/0/43 |
| MH4 разовый | IEK 130200305_ Петля LOOP | 210 000 шт одной накладной 09.06.2025 |
| MH4 разовый | IEK 010500008_ ВА47-29 25А | 7 488 шт 02.09.2026, это 2.9× второй по величине строки |
| MH4 паллеты | SE 030300013_ У-195 | Паллеты 6 480 и 10 908 при помесячных 108 и 0 |
...
| 4:00–5:00 | README, проверка на чистом клоне, деплой на Northflank | Ф, Б |
