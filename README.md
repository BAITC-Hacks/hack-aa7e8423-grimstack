...
Docker выбирает поддерживаемый вариант базового образа для своей платформы. Именованный том `grimstack-var` сохраняет утверждения из `var/approvals.json` после остановки контейнера. При повторном запуске достаточно второй команды.
...
- Прогоны и загруженные наборы хранятся в памяти одного процесса и пропадают после перезапуска. Утверждения записываются в `var/approvals.json`; учётных записей и авторизации нет.
